...
class ConditionalGenerator(nn.Module):
    ...
    def forward(self, noise, labels):
        ...
        embedded_labels = embedded_labels.view(embedded_labels.size(0), self.embed_dim, 1, 1)
...
def train_conditional_gan(dataloader, num_epochs=50, lr=0.0002, beta1=0.5, nz=100):
    ...
    for epoch in range(num_epochs):
        for i, (data, class_labels) in enumerate(dataloader):
            ...
            # Real/fake labels
            real_adv_labels = torch.full((b_size,), real_label, dtype=torch.float, device=device)
            ...
            errD_real_class = classification_criterion(real_class_output, real_class_labels)
            ...
            fake_adv_labels = torch.full((b_size,), fake_label, dtype=torch.float, device=device)
            fake_adv_output, fake_class_output = netD(fake_images.detach(), fake_class_labels)
            
            # Gubici za lažne slike
            errD_fake_adv = adversarial_criterion(fake_adv_output, fake_adv_labels)
            errD_fake_class = classification_criterion(fake_class_output, fake_class_labels)
            ...
            real_adv_labels_for_gen = torch.full((b_size,), real_label, dtype=torch.float, device=device)
...
def main():
    ...
    if len(dataset) == 0:
        print("Nisu pronađene slike u skupu podataka. Molimo dodajte slike listova biljaka u direktorijum sa podacima.")
        return
    ...
    print("Modeli su sačuvani kao 'conditional_generator.pth' i 'conditional_discriminator.pth'")
